...
public enum TicketBook {
	Logic;
		...
		Map<Integer, Passenger> passenger_data =new HashMap<Integer, Passenger>();
	...
	public void bookTicket(Passenger p) {
		...
		public void cancelTicket(int p_id){
			if(!passenger_data.containsKey(p_id)) {
				...
				if(p_alloted.equalsIgnoreCase("RAC")||p_alloted.equalsIgnoreCase("Berth"+"RAC")) {
					...
					if(wait_List.size() > 0) {
						Passenger passengerInwl = passenger_data.get(wait_List.poll());
				...
				if(rac_List.size() > 0) {
					Passenger passengerInRAC = passenger_data.get(rac_List.poll());
		...
		public void printBookedTicket(Passenger p, String allotedBerth) {
		...
		public void printRacTicket(Passenger p, String RACBerth) {
			p.alloted = RACBerth;
			passenger_data.put(p.passengerId, p);
		...
		public void printWlTicket(Passenger p, String WLBerth) {
			p.alloted = WLBerth;
			passenger_data.put(p.passengerId, p);
